...
class TicketSchema(Schema):
    ...
    quantity = fields.Integer(required=True, validate=validate.Range(min=1, max=10**11-1))
    address = fields.String(required=True, validate=validate.Length(min=1, max=255))
    price = fields.Integer(required=True, validate=validate.Range(min=1, max=10**20-1))
